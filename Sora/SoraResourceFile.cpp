...
namespace sora {
	...
	SoraResourceFileAuto::SoraResourceFileAuto(const util::String& file, bool retain):
	mRetain(retain),
    mName(file) {
    ...
    SoraResourceFileAuto::SoraResourceFileAuto(const util::String& file, ulong32 size, bool retain):
    mRetain(retain),
    mName(file)  {
    ...
    SoraResourceFileAuto& SoraResourceFileAuto::operator=(const util::String& file) {
        mData = SORA->getResourceFile(file, mSize);
		if(!mData)
			log_mssg(vamssg("Error loading resource file: %s", ws2s(file).c_str()));
        mRetain = false;
