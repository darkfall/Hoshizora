...
namespace flower {
    ...
    int32 LocalPlayer::update(float dt) {
        switch(mControlMode) {
    ...
    void LocalPlayer::sendMessage(sora::SoraMessageEvent* mssg) {
        if(mssg->getMessage() == SID_EVENT_ITEM_GET) {
            addScore(mssg->getData<int>());
        } 
        else {
    ...
    void LocalPlayer::accelerometerUpdate() {
        ...
        float speed = - mMaxSpeed * (y > 0 ? y + ConfigController::AccelerometerSensetivity :
                                             y - ConfigController::AccelerometerSensetivity);
