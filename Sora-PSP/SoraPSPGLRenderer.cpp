...
namespace sora{
	...
	void SoraPSPGLRenderer::_glInitialize() {
	...
	void SoraPSPGLRenderer::_glBeginFrame() {
	}
	
	void SoraPSPGLRenderer::_glEndFrame() {
		glFlush();
	}
	...
	void SoraPSPGLRenderer::_glSetProjectionMatrix(int32 w, int32 h) {
		glViewport(0, 0, w, h);
	...
	void SoraPSPGLRenderer::applyTransform() {
        if(!pCurTarget) {
            glViewport(0, 0,
	...
	void SoraPSPGLRenderer::_glBeginScene(ulong32 color, ulong32 t) {
		int32 width = _oglWindowInfo.width;
		int32 height = _oglWindowInfo.height;
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);
		
		if(t) {
			pCurTarget = (SoraRenderTargetPSPGL*)t;
	...
	void SoraPSPGLRenderer::_glEndScene() {
		flush();
		if(pCurTarget != NULL) {
            glFlush();
	...
	void SoraPSPGLRenderer::_glSetBlendMode(int32 blend) {
	...
	int32 SoraPSPGLRenderer::_glTextureGetWidth(ulong32 tex, bool bOriginal) {
		/*GLint w; 
	...
	int32 SoraPSPGLRenderer::_glTextureGetHeight(ulong32 tex, bool bOriginal) {
		/*GLint h; 
		glBindTexture(GL_TEXTURE_2D, tex);
	...
	void SoraPSPGLRenderer::bindTexture(SoraTexture* tex) {
        if(!tex) {
			flush();
	...
	int32 SoraPSPGLRenderer::_modeToGLMode(int32 mode) {
		switch (mode) {
	...
	void SoraPSPGLRenderer::renderTriple(SoraTriple& trip) {
		...
		if(currShader) {
			if(!currShader->attachShaderList())
				SoraCore::Instance()->log("SoraPSPGLRenderer: error attaching shader list");
			flush();
		}
	...
	bool SoraPSPGLRenderer::_glVersionCheck() {
	...
	bool SoraPSPGLRenderer::_glShaderCheck() {
		// to do
		// check extensions
	...
	inline bool SoraPSPGLRenderer::_glCheckError() {
