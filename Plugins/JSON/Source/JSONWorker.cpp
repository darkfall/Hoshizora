...
inline json_char FromOctal(const json_char * & str){
    ...
    return (json_char)((top << 6) | (middle << 3) | ((json_uchar)(*str - 48)));
}
...
void JSONWorker::DoArray(const internalJSONNode * parent, const json_string & value_t){
    ...
    for(size_t ending = FindNextRelevant(JSON_TEXT(','), value_t, 1);
	   ...
	   #ifdef JSON_SAFE
		  newValue = value_t.substr(starting, ending - starting);
	   ...
	   #else
		  NewNode(parent, EMPTY_JSON_STRING, value_t.substr(starting, ending - starting), true);
	   #endif
		  starting = ending + 1;
    }
    ...
    #ifdef JSON_SAFE
	   newValue = value_t.substr(starting, len - starting - 1);
	   JSON_ASSERT_SAFE(FindNextRelevant(JSON_TEXT(':'), newValue, 0) == json_string::npos, JSON_TEXT("Key/Value pairs are not allowed in arrays"), parent -> Nullify(); return;);
	   NewNode(parent, EMPTY_JSON_STRING, newValue, true);
    #else
	   NewNode(parent, EMPTY_JSON_STRING, value_t.substr(starting, len - starting - 1), true);
    #endif
}
...
void JSONWorker::DoNode(const internalJSONNode * parent, const json_string & value_t){
    ...
    json_string name = value_t.substr(1, name_ending - 2);	  //pull the name out
