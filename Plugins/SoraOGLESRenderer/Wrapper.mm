...
namespace sora {
	...
	string getPath(const char* s, const char* directory) {
		string fullPath(s);
		string sDir;
	...
	string NSString2String(NSString* nss) {
		string buffer([nss UTF8String]);
		return buffer;
	}
	
	NSString* string2NSString(const string& str) {
	...
	string getPathInAppDocDir(const string& file) {
		string buffer(NSString2String(applicationDocumentsDirectory()));
		buffer += "/";
		buffer += file;
		return buffer;
	}
...
SoraTexture *mTextureCreate(const char* fileName, bool isFullPath) {
	...
	if(!isFullPath) {
		fullPath = fileName;
		if(fullPath.find('/') != string::npos) {
			string sDir = fullPath.substr(0, fullPath.rfind('/'));
			string sName = fullPath.substr(fullPath.rfind('/')+1, fullPath.size());
