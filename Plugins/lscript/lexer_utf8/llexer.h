...
public:
	typedef std::map<std::wstring, int> OPERATOR_MAP;
	typedef std::map<std::wstring, int>::iterator OPERATOR_MAP_IT;
	OPERATOR_MAP operators;
	
	void add(const std::wstring& str, int id) {
		operators[str] = id;
	}
	int check(const std::wstring& str) {
...
public:
	...
	*/
	 ...
	 */
	const wchar_t* getCurrLexemeW() const;
	...
	*/
	 ...
	 */
	const wchar_t*	getCurrLine() const;
	/*
	 ...
	 */
	void addOperator(const std::wstring& op, int id);
...
private:
	...
	typedef std::vector<std::wstring> SOURCE_CONT;
