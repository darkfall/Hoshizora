...
int lAnmPacker::pack(const char* pstrScript, const char* pstrDest) {
	...
	if(!err) {
		...
		uint32 defaultNameHash = BKDRHash(nextAnmNameToken(lexer, err));
		...
		for(uint8 i=0; i<anmSize; ++i) {
			uint32 nameHash = BKDRHash(nextAnmNameToken(lexer, err));
...
int lAnmPacker::pack2(const char* pstrTex, const char* pstrDest) {
	...
	err = writeAnmT(pf, BKDRHash(defaultName.c_str()));
	...
	if(!bPaused && totalTime != 0) {
		if(currTime >= distTime) {
			...
			if(currAnmIndex >= anmNodes[currNodeId].texList.size()) {
				currAnmIndex = 0;
				if(!bLoop) {
					if(currId != defaultId && defaultId != -1) {
						currId = defaultId;
...
void lAnm::play(const char* name, bool loop) {
	int id = getNodeIdByAnmId(BKDRHash(name));
	if(id == -1)
		return;
