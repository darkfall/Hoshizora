...
namespace sora{
	...
	static int myGLfwWindowCloseFunc(GLFWwindow window) {
		SORA->shutDown();
		return 0;
	}
...
//		glDisable(GL_ALPHA_TEST);
	...
	SoraWindowHandle SoraOGLRenderer::createWindow(SoraWindowInfoBase* windowInfo) {
		glfwInit();
		_window = glfwOpenWindow(windowInfo->getWindowWidth(), 
					   windowInfo->getWindowHeight(), 
					   windowInfo->isWindowed()?GLFW_WINDOWED:GLFW_FULLSCREEN,
					   windowInfo->getWindowName().c_str(),
					   NULL);
		if(!_window)
			return 0;
		
		glfwMakeWindowCurrent(_window);
		
		if(windowInfo->getWindowPosX() != 0.f && windowInfo->getWindowPosY() != 0.f)
			glfwSetWindowPos(_window, windowInfo->getWindowPosX(), windowInfo->getWindowPosY());
		glfwSetWindowCloseCallback(myGLfwWindowCloseFunc);
...
//		glfwSwapInterval(1);
		...
		if(mainWindow->hideMouse())
			glfwDisable(_window, GLFW_MOUSE_CURSOR);

		bFullscreen = !windowInfo->isWindowed();
		windowInfo->init();

		return (ulong32)_window;
	...
	void SoraOGLRenderer::setWindowTitle(const SoraWString& title) {
		glfwSetWindowTitle(_window, ws2s(title).c_str());
	}

	void SoraOGLRenderer::setWindowPos(int32 px, int32 py) {
		glfwSetWindowPos(_window, px, py);
	}
	...
	void SoraOGLRenderer::setFullscreen(bool flag) {
		...
		bFullscreen = flag;
		GLFWwindow newwindow = glfwOpenWindow(mainWindow->getWindowWidth(), 
								 mainWindow->getWindowHeight(), 
								 bFullscreen==true?GLFW_FULLSCREEN:GLFW_WINDOWED, 
