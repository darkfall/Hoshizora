...
bool rfMainWindow::renderFunc() {		
	...
	if(rfState != STATE_LOGO) {
		...
		if(stateCount != 0) {
			...
			if(pState) pState->render();
		}
	
		srgc::GCN_GLOBAL->gcnLogic();
		srgc::GCN_GLOBAL->gcnDraw();
	} else {
		if(pFont) {
...
void rfMainWindow::init() {
	...
	/* gui part */
	srgc::GCN_GLOBAL->initGUIChan();
	gcn::Container* pTop = new gcn::Container();
	pTop->setDimension(gcn::Rectangle(0, 0, 640, 480));
	srgc::GCN_GLOBAL->pGUIChan->setTop(pTop);
